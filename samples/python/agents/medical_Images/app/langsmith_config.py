"""LangSmith configuration module for A2A Medical Assistant Agent."""
...
DEFAULT_PROJECT_NAME = "a2a-medical-assistant"
DEFAULT_TAGS = ["agent_type:medical_assistant", "medical_assistant", "a2a-agent"]
...
def setup_langsmith_environment(project_name=DEFAULT_PROJECT_NAME):
    """Configure environment variables for LangSmith tracing for Medical Assistant.
    ...
    if not langsmith_config["LANGCHAIN_API_KEY"]:
        logger.info("⚠️ LANGCHAIN_API_KEY not found - LangSmith disabled for Medical Assistant")
        LANGSMITH_ENABLED = False
    ...
    try:
        ...
        logger.warning(f"⚠️ Error setting up LangSmith for Medical Assistant: {e}")
